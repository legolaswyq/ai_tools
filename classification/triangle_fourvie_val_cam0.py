...
def load_images(images):
    ...
    imgs_np = random.choices(imgs_np, k=4)
...
def find_pairs_1_cameras(pairs, cam_anchor):
    for file in cam_anchor:
        find_pair = [file]
        pairs.append(find_pair)
...
def prepare_train_val_dataset(crop_dir, barcode, train_num, val_num):
    ...
    # get training pair 
    three_pair = []
    three_pair = find_pairs_3_cameras(three_pair, cameras["1"], cameras["4"], cameras["8"], max_pairs=int(train_num / 4))
    three_pair = find_pairs_3_cameras(three_pair, cameras["4"], cameras["8"], cameras["1"], max_pairs=int(train_num / 4))
    three_pair = find_pairs_3_cameras(three_pair, cameras["8"], cameras["1"], cameras["4"], max_pairs=int(train_num / 4))
    three_pair = find_pairs_3_cameras(three_pair, cameras["2"], cameras["6"], cameras["7"], max_pairs=int(train_num / 2))
    three_pair = find_pairs_3_cameras(three_pair, cameras["6"], cameras["7"], cameras["2"], max_pairs=int(train_num / 2))
    three_pair = find_pairs_3_cameras(three_pair, cameras["7"], cameras["2"], cameras["6"], max_pairs=int(train_num / 2))
    three_pair = random.choices(three_pair, k=int(train_num / 2))
    

    two_pair = []
    two_pair = find_pairs_2_cameras(two_pair, cameras["4"], cameras["8"], max_pairs=int(train_num / 4))
    two_pair = find_pairs_2_cameras(two_pair, cameras["4"], cameras["1"], max_pairs=int(train_num / 4))
    two_pair = find_pairs_2_cameras(two_pair, cameras["8"], cameras["1"], max_pairs=int(train_num / 4))
    two_pair = find_pairs_2_cameras(two_pair, cameras["6"], cameras["7"], max_pairs=int(train_num / 2))
    two_pair = find_pairs_2_cameras(two_pair, cameras["7"], cameras["2"], max_pairs=int(train_num / 2))
    two_pair = find_pairs_2_cameras(two_pair, cameras["6"], cameras["2"], max_pairs=int(train_num / 2))
    two_pair = random.choices(two_pair, k=int(train_num / 2))
    ...
    for val in vals:
        val_pair.append([val])
    val_pair = random.choices(val_pair, k=val_num)
